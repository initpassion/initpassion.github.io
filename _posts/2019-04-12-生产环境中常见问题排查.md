...
```
getstatic com.**.BusinessStrategySetCache businessStrategySetMap 'entrySet().iterator.{? #this.key=="cd04ad70ee8a430b96bfac7995f0b5b6"}'
```
...
```
 watch com.**.StatisticItemConfigCache queryOne  "{params, target, returnObj}" -n 3
...
watch com.**.RuleValidateController parseSubMetaInfoValueFromJSONObject "{params[0],params[1], params[2],throwExp}"   -x 2 -e
```
